...
window.addEventListener('load', function () {
    ...
    gameWindow.addEventListener('mousemove', function (evt) {
        if (!toggle_show)
            return;
        coorDOM = document.getElementsByClassName("coorbox")[0].innerHTML;
        //console.log(coorDOM);
        coordsXY = coorDOM.split(/(\d+)/);
        //console.log(coordsXY);
        x_new = (coordsXY[0].substring(1) + coordsXY[1])*1
        y_new = (coordsXY[2].substring(1) + coordsXY[3])*1;
...
function updateloop() {
    ...
    var url = window.baseTepmlateUrl + "data.php?" + new Date().getTime();
...
function loadTemplates() {
    ...
    if (needed_templates.length == 0) {
        if (zooming_in == false && zooming_out == false) {
            document.getElementById("minimap-box").style.display = "none";
            document.getElementById("minimap-text").style.display = "block";
            document.getElementById("minimap-text").innerHTML = "|  Nada aqui  |";
        }
    } else {
        document.getElementById("minimap-box").style.display = "block";
        document.getElementById("minimap-text").style.display = "none";
...
function drawTemplates() {
    ...
    for (i = 0; i < needed_templates.length; i++) {
        ...
        console.log(template+".png [Pos:", template_list[template]["x"], template_list[template]["y"]+"]");
    }
}
